...
def cadastro_de_diagnostico(page: ft.Page):
    ...
    def cadastrar_diagnostico(e):
        ...
        if not verificar_chamado:
            dlg_erros.content = ft.Text("Chamado não encontrada", color="red", size=20)
            page.open(dlg_erros)
            dlg_erros.open = True
            return
...
def alterar_diagnostico(page: ft.Page):
    ...
    def buscar_diagnostico(e):
        if not all([id_diagnostico, id_chamado, categoria, sintoma, manutencao]):
            dlg_erros.content = ft.Text("Preencha todos os campos", color="red", size=20)
            page.open(dlg_erros)
            dlg_erros.open = True
            return
        ...
        if not diagnostico:
            ...
            diagnostico.manutencao = manutencao.value
            session.commit()
    ...
    botao_alterar = criar_botao("Alterar", ft.Icons.ADD, buscar_diagnostico, ft.Colors.TEAL_700)
